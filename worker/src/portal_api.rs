    ...
    serde_json::json!([
        {
            ...
            "description": "A multi-ralph engineering team with recurring documentation review, feature management, research planning, cohesiveness review, implementation, test coverage, dependency updates, and status reporting tasks.",
            "ralphs": get_individual_ralphs()
        }
    ])
}

/// All individual ralphs available for standalone deployment.
fn get_individual_ralphs() -> serde_json::Value {
    serde_json::json!([
        {
            "name": "docs",
        ...
        {
            "name": "test-coverage",
            "task_file": "test-coverage-tasks.json",
            "state_file": ".test-coverage-state.json",
            "description": "Analyzes test coverage and writes tests for uncovered areas"
        },
        {
            "name": "dependency-updates",
            "task_file": "dependency-updates-tasks.json",
            "state_file": ".dependency-updates-state.json",
            "description": "Monitors and applies dependency updates and security patches"
        },
        {
            "name": "reporter",
...
/// `GET /api/portal/ralphs` — return all individual ralphs available for
/// standalone deployment (not bundled as a template).
async fn list_individual_ralphs(_req: Request, _ctx: RouteContext<()>) -> Result<Response> {
    json_response(&get_individual_ralphs(), 200)
}
